...
if ($_SERVER['REQUEST_METHOD'] == 'GET') :
    ...
    if(isset($_GET['id'])){
        ...
        if($id == 0){
            $sql = "SELECT * FROM `capo_azz_bet`";
            $query = mysqli_query($connection, $sql);

            $teamsDict = $query->fetch_all(MYSQLI_ASSOC);
            $response = ["capo_azz_bet" => $teamsDict];

            sendJson(200, '', $response);
        }else{
            $sql = "SELECT * FROM `capo_azz_bet` WHERE `id`='$id' AND `user_id`='$loggedUserId'";
            $query = mysqli_query($connection, $sql);
        ...
        $sql = "SELECT * FROM `capo_azz_bet` WHERE `user_id`='$loggedUserId'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') :
    ...
    $sql = "INSERT INTO `capo_azz_bet` (`user_id`,`bet_num`,`value`) VALUES($user_id,$bet_num,'$value')";
    $query = mysqli_query($connection, $sql);

    if ($query) sendJson(200, 'You have successfully registered a capoazzbet.');
...
if ($_SERVER['REQUEST_METHOD'] == 'PUT') :
    ...
    $sql = "SELECT `id` FROM `capo_azz_bet` WHERE `id`= $id";
    $query = mysqli_query($connection, $sql);
    ...
    if ($query) sendJson(200, 'You have successfully updated a capoazz bet.');
    sendJson(500, 'Something going wrong.');
...
if ($_SERVER['REQUEST_METHOD'] == 'DELETE') :
    ...
    $sql = "DELETE FROM `capo_azz_bet` WHERE `id` = $id";
    $query = mysqli_query($connection, $sql);

    if ($query) sendJson(200, 'You have successfully deleted a capoazz bet.');
    sendJson(500, 'Something going wrong.');
endif;
