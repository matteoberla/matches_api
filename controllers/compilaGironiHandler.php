...
class CompilaGironiHandler {

    public function calcPosGironi($connection, $currentUserId, $girone){
        ...
        //verifica ordinamento punti finch� non ci sono pi� sistemazioni da fare
        $has_changes = false;
        do{
            $has_changes = false;
            $prevKey = null;
            ...
            foreach ($pointsDict as $key => $row)
            {
                ...
                // in caso di pareggio: https://www.fifa.com/it/tournaments/mens/worldcup/canadamexicousa2026/articles/fase-a-gironi-mondiali-2026-critieri-qualificazioni
                ...
                if($prevTeamInfo["pts"] == $currentTeamInfo["pts"]){
                    ...
                    if(in_array($currentTeamInfo["team_id"], $pointsDict[$prevKey]["win"])){
                        //se squadra corrente � nelle vittorie della squadra precedente -> ha vinto la squadra precedente
                        $pointsDict[$prevKey]["order"] += 1;
                    }else if(in_array($prevTeamInfo["team_id"], $pointsDict[$key]["win"])){
                        //se squadra precente � nelle vittorie della squadra corrente -> ha vinto la squadra corrente
